#!/root/Downloads/miniconda3/bin/python
# -*- coding: utf-8 -*-
import os,glob,re
import cmaps
import numpy as np
...
def plot_uv_vvp(statid,times, hgt, u, v, wsp, vsp, saveFullname):
	...
	#plt.rcParams['axes.unicode_minus']=False
	# cnfont = {'fontname':'SimHei'} # 楷体: KaiTi 黑体: SimHei 仿宋: FangSong
	ax.set_title(statid,loc='left',fontsize=12, )
	#plt.xlabel('TIME',fontsize=6)
	#plt.ylabel('ALT:M',fontsize=6,rotation='horizontal',position=(0.2,1.01) ) #rotation='horizontal',position=(0.2,0.01) 
	ax.text(-0.02, 1.012,'ALT:M',verticalalignment='center',horizontalalignment='center',transform=ax.transAxes,color='k', fontsize=6.5)
	#  x, y range
...
def postLasttime(statid):
	...
	str_ymd = '/' + yy + '/' + mm + '/' + dd + '/'
	# -- 1hr ago --
	time_1hr = (datetime.strptime(time,'%Y%m%d%H%M%S') - timedelta(hours=1)).strftime("%Y%m%d%H%M%S")
	yy_1hr = time_1hr[0:4]
	mm_1hr = time_1hr[4:6]
	dd_1hr = time_1hr[6:8]
