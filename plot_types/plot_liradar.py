...
def plot_fig(element, ele_val, times, savepath):
	...
	fig, ax = plt.subplots(figsize=(8,8),dpi=120) #figsize=(14,10)
	...
	# save figure
	full_path = savepath + element + '/' + times[-1].strftime("%Y%m%d")
	if not os.path.exists(full_path):
		os.makedirs(full_path)
	fig.savefig(savepath + element + '/' + times[-1].strftime("%Y%m%d") +'/'+times[-1].strftime("%Y%m%d%H%M%S") +'.png',bbox_inches='tight') #,pad_inches=0.06
...
if __name__ == '__main__':
	...
	path="v:/"       # source data directory
	savepath="e:/radar_fig/decimal/"
	path_list=os.listdir(path)
	new_list = [ path_list[i.startswith('5')] for i in path_list ]
	new_list = []
	for i in path_list:
		if i.startswith('5'):
			new_list = new_list + [ i ]  
